...
const { extract, archive, textSubstitutions, navSubstitutions, opfSubstitutions } = require('./edition')
...
module.exports = async config => {
  ...
  // Automatically edit epub: apply substitutions
  if (config.textSubstitutions || config.navSubstitutions || config.opfSubstitutions) {
    let editionTempPath = path.join(tempPath, "edition")
    await extract(editionTempPath, epubFile)
    ...
    await archive(editionTempPath, epubFile)
}
